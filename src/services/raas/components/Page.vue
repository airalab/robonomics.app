  ...
  <Page>
    <RCard class="section-centered">
      ...
      <div class="row" style="margin:20px 0">
        <div class="col-md-12">
          <span style="color:#67ab9f;font-size: 24px;font-weight: bold;">{{success}}</span>
      ...
      <div class="row" style="margin:20px 0">
        <div class="col-md-6">
          <span style="color:#67ab9f;font-size: 24px;font-weight: bold;">{{fromNow}}</span>
          <br />
          <a href>last transaction</a>
        </div>
        <div class="col-md-6">
          <span style="color:#66b2ff;font-size: 24px;font-weight: bold;">{{countTx}}</span>
      ...
      <div class="row" style="margin:20px 0">
        <div class="col-md-12">
          <span style="color:#67ab9f;font-size: 24px;font-weight: bold;">{{cost}} $</span>
        </div>
      </div>
      ...
      <ul>
        <li v-for="(log, k) in last" :key="k">
          ...
          {{log.action}}
          <br />
          <a :href="`https://ipfs.io/ipfs/${log.data}`" target="_blank">{{log.data}}</a>
        </li>
      </ul>
    </RCard>
  </Page>
...
export default {
  ...
  created() {
    ...
    this.update();
    setInterval(() => {
      this.fromNow = moment(this.lastTxTime, 'x').locale(this.$i18n.locale).fromNow();
    }, 5000)
  },
  methods: {
    update() {
