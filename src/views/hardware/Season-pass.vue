      ...
      <div class="seasonpass-content">
        <div class="seasonpass-content-inside">
          <div class="seasonpass-msg" v-if="status !== 'START'">
            <template v-if="status === 'NO_ETH_EXTENSION'"
              >Connect or install Ethereum extension in your browser</template
            >
            <template v-if="status === 'WRONG_ETH_NETWORK'"
              >Please, switch to {{ chains[workingchain].name }}</template
            >
            <template v-if="status === 'NO_WALLET_PERMISSION'"
              >Please, check permissions in the connected extension</template
            >
            <template v-if="status === 'NO_ETH_ACCOUNT'"
              >No Ethereum account found: check your extension</template
            >
            <template
              v-if="status === 'NFT_NOT_FOUND' || status === 'TOKENS_NOT_FOUND'"
              >No Season Pass NFT accosiated with this account</template
            >
          </div>
          ...
          <div
            class="tokens"
            :class="'tokens-show-' + curentslide"
            v-if="status === 'TOKENS_FOUND' && tokens"
          >
            <div class="token" v-for="token in tokens" :key="token.tokenId">
              ...
              <robo-section offset="x1">
                <robo-button
                  v-if="token.tokenId && !token.activated"
                  @click.prevent="activateToken(token.tokenId)"
                  size="small"
                  >Activate</robo-button
                >
              </robo-section>
            </div>
          </div>
          <div
            class="tokens-switch"
            :class="'tokens-switch-showed-' + curentslide"
            v-if="status === 'TOKENS_FOUND'"
          >
            <robo-button
              v-for="(token, i) in tokens"
              :key="token.tokenId"
              clean
              @click.prevent="setslide(i + 1)"
              >{{ i + 1 }}</robo-button
            >
          </div>
        </div>
        <div
          class="seasonpass-monkey"
          aria-hidden="true"
          v-if="status !== 'TOKENS_FOUND'"
        >
...
mipdstore.subscribe((providers) => {
  logger.log("Providers updated:", providers);
...
const provider = computed(() => {
  return state.providers.find(
    (i) =>
      i.info.rdns === store.state.robonomicsUIvue.ethereum.activeProviderRdns
  );
});
...
const isConnected = () => {
  if (provider?.value) {
    return (
      provider.value?.provider?._state?.isConnected ||
      provider.value?.provider?.isConnected
    );
  } else {
    return false;
  }
...
const getNFT = async (blockupdate = false) => {
  ...
  try {
    if (!blockupdate) {
      const perms = await provider.value.provider.request({
      ...
      const accounts = await provider.value.provider.request({
      ...
      const chain = await provider.value.provider.request({
        method: "eth_chainId",
        params: [{ eth_accounts: {} }]
      });
...
const loadTokens = async () => {
  ...
  for (let index = 0; index < balance.value; index++) {
    try {
      const tokenId = await contract.tokenOfOwnerByIndex(signer.address, index);
