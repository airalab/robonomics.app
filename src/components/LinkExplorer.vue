...
export default {
  ...
  computed: {
    href: function () {
      return (this.type === 'chain') ? filters.urlExplorer(this.text, this.category) : filters.urlIpfs(this.text, this.category)
    },
    label: function () {
      return (this.isSlice) ? (this.text.slice(0, 6) + '...' + this.text.slice(-4)) : this.text
    }
  },
  methods: {
    icon() {
